...
3. 视频开头出现了一个球状透明生物，体表有向外延伸的白色凸起结构
视觉引导：请在脑海中构建一个画面：视频最开始，一个近乎全透明的球状生物悬浮在深暗的海水中。球体表面分布着白色的凸起结构，从球体表面向外延伸隆起，在透明躯体的映衬下清晰可见。
语言引导：请记住这个要点：视频最先出现的生物形态与特征——球状、近乎透明、体表有向外延伸的白色凸起结构。
...
语言引导：请记住这个要点：视频中海洋生物的种类总数——总共出现6种形态各异的生物。

8. 视频最后的蓝色光芒出现时，亮度始终保持不变
视觉引导：请在脑海中构建一个画面：在漆黑深海的背景中，一道蓝色光芒亮起。光线稳定持续，亮度从出现到消失始终保持一致，没有发生任何增强或减弱的变化。
语言引导：请记住这个要点：蓝色光芒的发光特征——亮度始终不变。

9. 纺锤状生物摆动的侧翼呈现出薄膜瓣状
视觉引导：请在脑海中构建一个画面：纺锤状生物在水体中向前游动。身体两侧各有一片侧翼，在游动过程中交替展开和收拢，展开时呈现出薄而透明的瓣状形态。
语言引导：请记住这个要点：纺锤状生物的游动结构特征——身体两侧生有侧翼，摆动时呈薄膜瓣状。
...
11. 前端圆润、后部收窄的半透明白色生物有2个侧足
视觉引导：请在脑海中构建一个画面：一种半透明白色生物悬浮在水中。它的身体前端圆润，后端逐渐收窄。身体两侧各伸出一条侧足，左右对称分布，在浮动过程中清晰可辨。
语言引导：请记住这个要点：半透明白色生物的形态与侧足数量——身体前端圆润、后部收窄，共有2根侧足。

12. 球状生物体表有白色透明薄膜结构
视觉引导：请在脑海中构建一个画面：球状生物悬浮在深色海水中。它的体表覆盖着一层白色透明的薄膜，包裹在球体表面，在深色背景的映衬下轮廓清晰可见。
语言引导：请记住这个要点：球状生物体表的结构特征——体表覆盖有白色透明薄膜。
...
14. 水母伞体整体呈现半透明带颜色
视觉引导：请在脑海中构建一个画面：水母的伞体呈半透明状。透过伞体可以隐约看到内部结构轮廓，同时伞体本身带有一定的颜色，在深色水体中形成可见的色调。
语言引导：请记住这个要点：水母伞体的透明度与颜色特征——伞体呈半透明状态，且带有颜色。

15. 视频中出现过一种顶部为膜状带褶皱，能张合的橙红色生物
视觉引导：请在脑海中构建一个画面：一只橙红色的生物竖立在海水中。其顶部边缘呈膜状，带有褶皱，在运动过程中可见其顶部反复张开与合拢。
语言引导：请记住这个要点：橙红色生物的顶部结构特征——顶部呈膜状带褶皱，且能张合。

16. 视频中出现过一个橙红色生物，呈上宽下窄的桶形
视觉引导：请在脑海中构建一个画面：一只橙红色的生物悬浮在水中。其整体轮廓从上部到下部逐渐收窄，呈桶形结构，上端明显宽于下端。
语言引导：请记住这个要点：橙红色生物的整体轮廓——上宽下窄的桶形。
...
18. 条状生物嘴部前端尖锐细长
视觉引导：请在脑海中构建一个画面：条状生物的嘴部位于头部最前方，嘴部前端呈锥形收窄，延伸出细长的尖细结构，在头部前方清晰可见。
语言引导：请记住这个要点：条状生物的嘴部形态——嘴部前端尖锐细长。
...
20. 蜘蛛状生物有八条用于游动的足
视觉引导：请在脑海中构建一个画面：蜘蛛状生物悬浮在海中，八条细长的足从躯干周围向四面八方伸展开来，在游动过程中交替划动，在空间上形成均匀的放射状分布。
语言引导：请记住这个要点：蜘蛛状生物游动的足的数量——该生物共有8条用于游动的足。
